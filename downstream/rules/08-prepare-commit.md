...
- Do not prepare main-task commits or task-scoped/tracked-feature-scoped ad hoc `feat` commits for paused, completed, or archived tracked features unless the user explicitly asks for an exception
- For `management`, an explicit tracked-feature selector may reference a `planned`, non-active, completed, or archived tracked feature without activating it
